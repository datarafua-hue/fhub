          ...
          {isEditing ? (
            ...
            <>
              ...
              <div className="flex items-center gap-3 mt-3 flex-wrap">
                {/* Translate button - show if comment is in different language */}
                {needsTranslation && (
                  <button
                    onClick={handleTranslate}
                    disabled={isTranslating}
                    className="text-sm text-gray-400 hover:text-accent-blue transition-colors flex items-center gap-1"
                  >
                    {isTranslating ? (
                      <>
                        ...
                        {currentLanguage === 'ru' ? 'Перевод...' : 'Translating...'}
                      </>
                    ) : showTranslation ? (
                      <>
                        ...
                        {commentLanguage === 'ru' ? 'See Russian' : 'See English'}
                      </>
                    ) : (
                      <>
                        <svg className="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        ...
                        {currentLanguage === 'ru' ? 'Перевести на русский' : 'Translate to English'}
                      </>
                    )}
                  </button>
                )}
                {currentUserId && (
                  ...
                  >
                    {currentLanguage === 'ru' ? 'Ответить' : 'Reply'}
                  </button>
                )}
                {canEdit && (
                  <button
                ...
                {comment.replies && comment.replies.length > 0 && (
                  ...
                  >
                    {showReplies 
                      ? (currentLanguage === 'ru' ? 'Скрыть' : 'Hide')
                      : (currentLanguage === 'ru' ? 'Показать' : 'Show')
                    } {currentLanguage === 'ru' ? 'ответы' : 'replies'} ({comment.replies.length})
                  </button>
                )}
              </div>
            </>
